...
def add_subtitles_to_clip(
    ...
    # Create a temporary TextClip to measure the size of the text without wrapping
    ...
    # Get the actual width and height of the text without wrapping
    text_width, text_height = measuring_clip.size
    measuring_clip.close()  # Properly dispose of the measuring clip

    padding = 10
    subtitle_duration = subriptime_to_seconds(subtitle.end) - subriptime_to_seconds(subtitle.start)

    # Determine if the text needs wrapping
    if text_width + padding <= max_box_width:
        # If text is short, create a TextClip without wrapping
        ...
        # If text is long, create a TextClip with wrapping enabled
        subtitle_clip = TextClip(
            subtitle.text,
            fontsize=font_size,
            color=font_color,
            ...
            stroke_width=1,
            font=font_path,
            method="caption",  # Enables automatic wrapping of text
            align="center",
            size=(max_box_width, None)  # Width constrained by max_box_width, height auto
        ).set_duration(subtitle_duration)
        
        # Wrapped subtitle dimensions
        text_width, text_height = subtitle_clip.size
        
        # Recalculate the maximum width of the wrapped text lines
        wrapped_lines = subtitle.text.split("\n")
        max_real_width = max(
            [TextClip(line, fontsize=font_size, font=font_path).size[0] for line in wrapped_lines]
        )

        # Set box width based on maximum wrapped line width + padding
        box_width = min(max_real_width + padding, max_box_width)

    # Add padding and create the background box
    box_height = text_height + 2 * padding  # Box height with margin for top and bottom

    # Ensure bg_color is RGB
    bg_color = convert_color(bg_color)
    bg_opacity = 0.5  # Set the opacity of the background color
