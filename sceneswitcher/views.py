    ...
    if request.method == "POST":
	    ...
	    try:
	        ...
	        if not video_file or not mp3_file or not text_file or not font_file:
	            return JsonResponse({'status': 'error', 'message': 'Missing required files'},400)
	        ...
	        return JsonResponse({'status': 'error', 'message': f'An error occurred while saving files: {e}'},500)
	    # New parameters
	    ...
	    if not global_font_size or not global_box_color or not global_bg_color:
	        return JsonResponse({'status': 'error', 'message': 'Missing required form data'},400)
	    ...
	    try:
	        ...
	        return JsonResponse({'status': 'error', 'message': f'Failed to generate SRT file: {e}'},500)
...
def upload_new_scene(request):
    if request.method == 'POST':
	    ...
	    if not any(r['srt_index'] == srt_index for r in request.session['replacements']):
